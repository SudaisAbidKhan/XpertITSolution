...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  /* Raw trim first — validate, THEN sanitise for display only */
  $name    = trim($_POST['cont_name']    ?? '');
  $email   = trim($_POST['cont_email']   ?? '');
  $phone   = trim($_POST['cont_phone']   ?? '');
  $subject = trim($_POST['cont_subject'] ?? '');
  $message = trim($_POST['cont_message'] ?? '');
  $program = trim($_POST['cont_program'] ?? '');
  $type    = trim($_POST['cont_type']    ?? 'enrolment');

  if ($name && $email && $message && $subject && filter_var($email, FILTER_VALIDATE_EMAIL)) {
    try {
      $pdo->prepare(
        "INSERT INTO enquiries (name, email, phone, program, subject, message, type)
         VALUES (?, ?, ?, ?, ?, ?, ?)"
      )->execute([$name, $email, $phone, $program, $subject, $message, $type]);

      echo json_encode(['success' => true,  'msg' => "Thank you, " . htmlspecialchars($name) . "! We'll be in touch within one working day."]);
    } catch (Exception $e) {
      echo json_encode(['success' => false, 'msg' => 'Database error: ' . $e->getMessage()]);
    }
  } else {
    echo json_encode(['success' => false, 'msg' => 'Please fill in all required fields with a valid email address.']);
  }
  exit; /* Return JSON only — no HTML */
}
?>
...
<link rel="stylesheet" href="assets/css/contact.css">
  ...
  <section class="contMain" id="contForm">
    <div class="container contMain-inner">
      ...
      <div class="contForm-wrap">
        ...
        <!-- Alert boxes — shown/hidden by JS, no PHP needed -->
        <div class="contAlert contAlert--success" id="contAlertSuccess" style="display:none;">
          <span class="contAlert-icon">&#10003;</span>
          <p id="contAlertSuccessMsg"></p>
        </div>
        <div class="contAlert contAlert--error" id="contAlertError" style="display:none;">
          <span class="contAlert-icon">&#x26A0;</span>
          <p id="contAlertErrorMsg"></p>
        </div>

        <form class="contForm reveal d2" id="contFormEl" method="POST" action="." novalidate>

          <div class="contForm-row">
            <div class="contField">
              <label class="contField-label" for="cont_name">Full Name <span class="contField-req">*</span></label>
              <input class="contField-input" type="text" id="cont_name" name="cont_name" placeholder="e.g. Ahmad Raza" required />
            </div>
            <div class="contField">
              <label class="contField-label" for="cont_email">Email Address <span class="contField-req">*</span></label>
              <input class="contField-input" type="email" id="cont_email" name="cont_email" placeholder="e.g. ahmad@email.com" required />
            </div>
          </div>
          ...
          <div class="contForm-row">
            <div class="contField">
              ...
              <input class="contField-input" type="tel" id="cont_phone" name="cont_phone" placeholder="e.g. 0300 1234567" />
            </div>
            <div class="contField">
              ...
              <select class="contField-input contField-select" id="cont_program" name="cont_program">
                ...
                <option value="cloud">Cloud &amp; Networking</option>
          ...
          <div class="contField">
            ...
            <input class="contField-input" type="text" id="cont_subject" name="cont_subject" placeholder="e.g. Enrolment enquiry for January batch" required />
          </div>

          <div class="contField">
            <label class="contField-label" for="cont_message">Message <span class="contField-req">*</span></label>
            <textarea class="contField-input contField-textarea" id="cont_message" name="cont_message" rows="5" placeholder="Tell us a bit about yourself and what you're looking for…" required></textarea>
          </div>
          ...
          <fieldset class="contRadioGroup">
            ...
            <div class="contRadio-options">
              <label class="contRadio">
                ...
                <span class="contRadio-box"></span>Enrolment
              </label>
              <label class="contRadio">
                <input type="radio" name="cont_type" value="information" />
                <span class="contRadio-box"></span>General Information
              </label>
              <label class="contRadio">
                <input type="radio" name="cont_type" value="visit" />
                <span class="contRadio-box"></span>Campus Visit
              </label>
              <label class="contRadio">
                <input type="radio" name="cont_type" value="corporate" />
                <span class="contRadio-box"></span>Corporate / Partnership
              </label>
            </div>
          </fieldset>
      ...
      <aside class="contSidebar">
        ...
        <div class="contSidebar-card reveal d1">
          ...
          <p class="contSidebar-cardText">Book a free campus tour. See our labs, meet faculty, and get a feel for student life at Xpert IT Solution.</p>
        ...
        <div class="contSidebar-card contSidebar-card--green reveal d2">
          ...
          <p class="contSidebar-cardText">Prefer a quick chat? Message us on WhatsApp and get answers from our admissions team in minutes.</p>
        ...
        <div class="contSidebar-card reveal d4">
          ...
          <div class="contSocials">
            <a href="#" class="contSocial" aria-label="Facebook">
              <span class="contSocial-icon">f</span><span>Facebook</span>
            </a>
            <a href="#" class="contSocial" aria-label="Instagram">
              <span class="contSocial-icon">&#x1F4F7;</span><span>Instagram</span>
            </a>
            <a href="#" class="contSocial" aria-label="LinkedIn">
              <span class="contSocial-icon">in</span><span>LinkedIn</span>
            </a>
            <a href="#" class="contSocial" aria-label="YouTube">
              <span class="contSocial-icon">&#x25B6;</span><span>YouTube</span>
            </a>
          </div>
        </div>
  ...
  <section id="cta-band">
    <div class="container">
      <div class="cta-inner">
        <div class="cta-text">
          <h2 class="cta-heading reveal">Still thinking?<br/>Let's talk <em>today</em>.</h2>
  ...
  <script>
    ...
    const contFormEl      = document.getElementById('contFormEl');
    const contBtn         = document.querySelector('.contSubmit');
    const alertSuccess    = document.getElementById('contAlertSuccess');
    const alertError      = document.getElementById('contAlertError');
    const alertSuccessMsg = document.getElementById('contAlertSuccessMsg');
    const alertErrorMsg   = document.getElementById('contAlertErrorMsg');

    contFormEl.addEventListener('submit', async function(e) {
      ...
      try {
        const formData = new FormData(contFormEl);
        const response = await fetch('contact.php', { method: 'POST', body: formData });
        const result   = await response.json();

        if (result.success) {
          alertSuccessMsg.textContent = result.msg;
          alertSuccess.style.display  = 'flex';
          contFormEl.reset();
          alertSuccess.scrollIntoView({ behavior: 'smooth', block: 'center' });
        } else {
          alertErrorMsg.textContent = result.msg;
          alertError.style.display  = 'flex';
        }

      } catch (err) {
        alertErrorMsg.textContent = 'Something went wrong. Please try again.';
        alertError.style.display  = 'flex';
      }

      /* Restore button */
      contBtn.classList.remove('contSubmit--loading');
      contBtn.disabled = false;
    });

    /* ── SMOOTH SCROLL FOR HERO CHIPS ── */
